...
fifo::fifo(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), insertion_cycles(static_cast<std::size_t>(sets * ways), 0) {}
...
void fifo::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
